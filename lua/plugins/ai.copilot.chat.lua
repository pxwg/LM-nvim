...
package.path = package.path .. ";" .. vim.fn.stdpath("data") .. "/lazy/CopilotChat.nvim/lua/?.lua"
...
local function has_memory_dream_content(text)
  text = tostring(text or "")
  ...
  for _, line in ipairs(vim.split(text, "\n", { plain = true })) do
    ...
    if
      trimmed ~= ""
      and trimmed ~= "(no output)"
      and trimmed ~= "+++"
      and not trimmed:match("^<!%-%- Generated by memory%-dream%.")
      and not trimmed:match("^%[CopilotChat tool guard:")
      and not trimmed:match("^source%s*=")
    then
      return true
    end
  end
...
local function project_memory_resource(source)
  if vim.fn.executable("memory-dream") ~= 1 then
    return {}
  end

  local cwd = source_cwd(source) or vim.uv.cwd()
  local ok, result = pcall(bounded_system, { "memory-dream", "show" }, cwd, {
    timeout_ms = 3000,
    max_stdout_bytes = 30000,
    max_stderr_bytes = 4000,
  })
  if not ok or not result or result.code ~= 0 or not has_memory_dream_content(result.stdout) then
    return {}
  end

  return {
    {
      uri = "memory-dream://show",
      name = "Project Memory",
      mimetype = "text/markdown",
      data = table.concat({
        "# Project Memory",
        "",
        "Output of `memory-dream show` for the current working directory.",
        "Treat this as persistent project-level context and use it when relevant.",
        "",
        result.stdout,
      }, "\n"),
    },
...
local opts = {
  ...
  resources = { "selection", "alma_zk_workspace", "memory" },
  functions = {
    bash = {
      group = "copilot",
    ...
    memory = {
      group = "resource",
      uri = "memory-dream://show",
      description = "Attach project memory from `memory-dream show` for the current working directory. Empty or missing project memory resolves to no context.",
      schema = {
        type = "object",
        properties = {},
        required = {},
      ...
      resolve = function(_, source)
        return project_memory_resource(source)
      end,
    },
    save_copilot_chat = {
      group = "neovim",
